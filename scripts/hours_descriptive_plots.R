...
build_hours_descriptive_plots <- function(cleaned_df, hours_by_period = NULL) {
  ...
  p_year <- ggplot(
    ...
    # A blank layer pins the levels panel's range to at least 36-42 hours: spanning only the data
    # (about 38 to 41) lets movements of under half an hour fill the panel, and the gap panel
    # beneath, not the axis, should carry the visual claim (docs/decisions/grade-report-2-response.md,
    # Figure 1). The gap panel is unaffected: its own data define its range.
    geom_blank(
