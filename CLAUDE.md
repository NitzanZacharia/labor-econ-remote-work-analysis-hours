...
- `paper/tables/*.tex` are GENERATED by `main.R` (`build_paper_tables()` +
  `export_paper_tables()`) from the fitted models and are `\input{}` by `paper/paper.tex` inside
  its hand-written table floats. Never edit them by hand; change `scripts/build_paper_tables.R`
