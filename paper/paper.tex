...
(Figures~\ref{fig:pretrend} and~\ref{fig:pretrend-ddd}), all on the harmonized hours sample of
Section~\ref{sec:data}. The last of these is the test that matters for the headline estimate, and
it is also the weakest: with forty occupation clusters its pre-period standard errors are of the
same order as the effect being tested, so it cannot exclude a pre-trend the size of the estimate.
It is evidence of no visible violation, not proof of none.
...
anchored to 2021 rather than the 2020 the research plan specified, because no 2020 extract exists;
by 2021 occupations had had a year to adapt, unevenly. Section~\ref{sec:res-robust} sets out why
the calibration is unlikely to manufacture the result, but the fully pre-treatment external index
yields a positive and insignificant estimate, and a reader who weights that measure more heavily
than we do will read the headline as directionally supported rather than established.
...
call-ups, which bear on men's hours and so on the placebo, and school closures and evacuations,
...
shows that within-firm sorting matters as much as occupation for Israeli gender gaps.
...
\paragraph{Inference.}
The hours DDD clusters on forty occupation groups. The reported $p$-values carry
\texttt{fixest}'s default $t(G-1)$ reference distribution (Section~\ref{sec:inference}) but no
wild cluster bootstrap, so the occupation-clustered standard errors should be read as approximate.
The panel does not support within-person identification: only 1.9\% of respondents span the
pre/post boundary.
