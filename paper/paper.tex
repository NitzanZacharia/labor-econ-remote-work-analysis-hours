...
% Remaining "% TODO(<item>)" comments are author decisions (C3 citations, D3 earnings, J/D11).
...
$p<0.01$) --- about $1.7$ hours a week between the most and least teleworkable quartiles of
occupations --- while the implied change at zero exposure is if anything negative. The estimate survives
...
while the implied change for mothers at zero exposure is if anything negative. Scaled to the
regressor's actual dispersion, that is $0.64$ hours per standard deviation of exposure, or about
$1.7$ hours between the top and bottom quartiles of occupations --- the same order as the
$1.74$-hour intensive-margin penalty itself. The estimate survives a selection correction, two
age-balance specifications, the exclusion of the occupations whose exposure was calibrated on
post-period data, and the exclusion of 2023, and fathers show no positive exposure gradient. On the
employment margin the DiD is a precise zero and the DDD is too imprecise to be informative.
...
is applied, and Section~\ref{sec:limitations} says what that leaves open. Clustering two ways, by
...
$2.864$ hours per unit of exposure, or $0.57$ hours per standard deviation ($7.3\%$ of mean weekly
hours), so an effect of the size observed is detectable, though without a great deal of room to
spare; the reweighted specification of Section~\ref{sec:res-robust} shows what that margin means
in practice.
...
the calibration uses, so the two are different variables. The direct test is to drop the ten
swapped occupations and re-estimate on the thirty where the calibrated and external indices
coincide by construction, so that no post-period information enters the regressor at all. On that
sample the triple interaction is $4.309$\sym{**} (SE $1.192$, $N = 132{,}046$, 30 clusters) ---
larger than the headline, not smaller. The post-period calibration does not produce the result; if
anything the ten swapped occupations, which together account for nearly half the estimation sample,
dilute it. What the calibration cannot be made into is a pre-treatment measure, and
Section~\ref{sec:limitations} says so.
...
\paragraph{Sample and inference.}
...
Clustering two ways, by individual and by occupation, leaves the headline standard error unchanged
at $1.022$: the occupation-level dependence already dominates the individual-level one.
...
\midrule
\emph{Sample and inference} & & & \\
\quad Unswapped occupations only (30 of 40) & $4.309$\sym{**} & 1.192 & 132{,}046 \\
\quad Excluding survey year 2023 & $2.979$\sym{*} & 1.170 & 208{,}833 \\
\quad Two-way clustering, individual and occupation & $3.224$\sym{**} & 1.022 & 246{,}326 \\
\bottomrule
\end{tabular}
\begin{tablenotes}\small
\item All rows: weekly hours, women working in the reference week, standard controls, calibrated
exposure unless stated, clustered by occupation unless stated. The unswapped-occupations row drops
the ten occupations whose exposure was swapped to its realized 2022--23 value, so the calibrated
and external indices coincide on the rows that remain.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
The magnitude is economically meaningful where it applies. The regression implies a difference of
$1.7$ hours a week between mothers in the most and least teleworkable quartiles of occupations,
and the raw quartile differences of Figure~\ref{fig:dose-response} put the same contrast at $1.9$.
Set against the $1.74$-hour intensive-margin penalty of Table~\ref{tab:hours}, mothers in
teleworkable jobs recovered most of the penalty after 2021, and mothers elsewhere recovered none of
it. An hour and three-quarters a week is also the order of magnitude that two remote days' saved
commuting would free, which is the most literal form the time-constraint mechanism can take.
% TODO(C3): if Aksoy et al. (2023, AEA P&P) is verified and added to references.bib, cite their
% estimate of commute time saved per WFH day here in place of the unreferenced comparison.
...
innocuous for the triple interaction. It is related: absence runs at $14.4\%$ of the employed in the
least teleworkable quartile of occupations against $8.5$--$9.6\%$ in the other three, and among
mothers it fell after 2021 in the top quartile (from $12.1\%$ to $10.4\%$) while rising in the
bottom one (from $15.5\%$ to $16.9\%$).\footnote{Reference-week absence among employed women aged
25--59 with a matched occupation, by the quartiles of Figure~\ref{fig:dose-response}: $14.4\%$,
$8.5\%$, $9.5\%$ and $9.6\%$ from the least to the most teleworkable quartile. Among childless women
the top-quartile share is $6.1\%$ in both periods.} Movement across the absence margin is therefore
part of what selection into the observed-hours sample means in this design, which is why the bounds
of Section~\ref{sec:lee} define selection on that sample rather than on employment. They trim that
selection; they do not make the restriction innocuous by assumption.
...
difference --- and the estimate survives dropping the year: without 2023 the triple interaction is
$2.979$ (SE $1.170$), significant at the 5\% level rather than the 1\%, and the plain DiD remains a
null (Section~\ref{sec:res-robust}). The 2023 point should nonetheless be held more loosely than the
others. The extract carries no survey-month field after cleaning, so the exclusion is of the whole
year rather than of its final quarter. Early 2021 likewise included a third lockdown with partial
