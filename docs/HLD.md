...
| Gender Placebo Layer | `gender_placebo.R` | `run_gender_placebo(folder_path)` — reruns the secondary (employment) DiD on men (fathers vs. childless men) via `load_and_clean_data(..., sex_filter = "men")`, as a falsification check. Not called by default from `main.R` (manual invocation). No hours-based placebo analog exists yet. |
| WFH-Exposure & Mechanism (DDD) Layer | `wfh_exposure_index.R`, `wfh_exposure_cells.R`, `isco_masking_diagnostics.R`, `ddd_collinearity_diagnostics.R`, `ddd_regression.R`, `hours_ddd_regression.R`, `hours_ddd_lee_bounds.R`, `imbens_manski_ci.R` | Builds four separate occupation/cell-level WFH-exposure measures (external, calibrated, realized, cell-based shift-share) from `exposure_population_df` (women + men, not the women-only analysis sample — see §4.2), a sensitivity check for ISCO disclosure-masking, a runtime collinearity diagnostic, and two triple-interaction DDD regressions on the same exposure measures: the **primary DDD on hours** (`WorkHoursCont`, pure occupation-level exposure, plus a generalized Lee-bounds correction — see [`docs/decisions/hours-ddd-pivot.md`](decisions/hours-ddd-pivot.md)), and the **secondary DDD on employment** (`Employed`, cell-based exposure, two specs; three occupation-level robustness variants). Clustered on the exposure regressor's own assignment level (cell, or occupation), not `IDPUF`. |
| Age-Balance Robustness Layer | `robustness/balance_test.R`, `robustness/age_balance_robustness.R`, `robustness/pretrend_wald_test.R` | Pre-period covariate-balance test (Mother vs. non-Mother, by `WFH_Exposure` quartile), a diagnosed `GilNK` (age-group) imbalance concentrated in the lowest exposure quartile, two comparison DDD specs (age-interacted; `GilNK`-reweighted), and a joint Wald test on `Diagnostics.R`'s pre-trend coefficients. Off by default (`RUN_AGE_BALANCE_ROBUSTNESS` flag in `main.R`) — see [`docs/decisions/age-balance-robustness-chain.md`](decisions/age-balance-robustness-chain.md). `robustness/phase2_robustness.R` exists but is **not** wired into `main.R` (see §4.2). |
...
[   Stats     ] [   Margin Modeling   ] [                ] [              ] [  (manual run)  ]
 ...
 [ WFH-Exposure & DDD Layer: wfh_exposure_index.R / wfh_exposure_cells.R /
   ...
   +----------+----------+----------------------+
   v                     v                      v
[ Primary DDD (hours) ]  [ Secondary DDD (Employed) ]  [ 3 robustness occupation-level DDDs ]
              |
              v
   [ Age-Balance Robustness Chain: robustness/balance_test.R /
     age_balance_robustness.R / pretrend_wald_test.R ]  (off by default)
              |
              v
 [ Output/Export: export_results.R -> outputs/ (CSV + PNG) ]
```

`israeli_market_mismatch.R` / `run_mismatch.R` sit outside this default flow — a separate, manually-run descriptive exhibit against a cached `cleaned_df.rds`, sharing `wfh_exposure_cells.R`'s `calibrate_isco_exposure()`. `robustness/phase2_robustness.R` also sits outside this flow entirely — not sourced by `main.R` at all, pending the `MishkalSofi`-as-weight question in §4.2.
...
7. **Fan-out to analysis** (from `main.R`, unless noted as manual):
   ...
   - **Primary DDD (intensive margin, hours)** — `WorkHoursCont ~ Mother*Post*WFH_Exposure + controls`, pure occupation-level exposure (`exposure_calibrated`'s `wfh_exposure_calibrated`, joined by ISCO-2 code), `Employed==1` only, clustered on occupation code, plus a generalized Lee-bounds selection correction (`run_hours_ddd_lee_bounds()`) and an Imbens-Manski CI. Real-data result: `Mother:Post:WFH_Exposure` = 3.4073 (p<0.001), well outside its own MDE and Lee-bounds-robust — see `docs/decisions/hours-ddd-pivot.md` for full results. **Code/docs note:** `main.R` still gates this behind `RUN_HOURS_DDD_PIVOT <- FALSE` and its section-8g comments haven't been updated to reflect its new primacy — a separate follow-up code task, not yet done.
   - **Secondary DDD (extensive margin, employment)** — `Employed ~ Mother*Post*WFH_Exposure + Mother:GilNK + controls`, cell-based exposure, two specs (additive controls; interacted cell fixed effects — the standard fix for this shift-share regressor's collinearity with its own controls), clustered on the `(GilNK, TeudaGvoha, MachozMegurim)` cell. `Mother:GilNK` (added 2026-09-11) corrects for a confirmed pre-period age imbalance between mothers/non-mothers whose size varies by exposure quartile — see `docs/decisions/age-balance-robustness-chain.md`; it doesn't change the `Mother:Post:WFH_Exposure` conclusion but surfaces its own real age-heterogeneous-penalty finding. `check_spec1_collinearity()` reports the additive spec's collinearity at runtime; `check_for_dropped_coefficients()` guards both specs generically. This was the project's primary DDD before the hours pivot (`docs/decisions/hours-ddd-pivot.md`); still run and reported, no longer the headline result.
   - **Robustness DDDs** — `run_ddd_regression()` run three times (calibrated, external, realized occupation-level indices), each also reporting a dropped-vs-retained occupation exposure comparison for its second-stage mechanism regression. These are robustness variants of the secondary (employment) DDD; no occupation-level robustness variants exist yet for the primary (hours) DDD.
   - **Age-balance robustness chain** — `RUN_AGE_BALANCE_ROBUSTNESS` flag (default `FALSE`) in `main.R`: `run_balance_test()`, `diagnose_gilnk_by_quartile()`, `run_ddd_age_interacted()`, `run_ddd_reweighted()`, `run_pretrend_joint_test()`. See `docs/decisions/age-balance-robustness-chain.md` for the real-data verification of the `GilNK` imbalance this chain checks for. This chain was built against the secondary (employment) DDD; whether its age-balance/reweighting logic also needs to be ported to the primary (hours) DDD is an open question the pivot memo does not address.
...
| `ddd_regression.R` | `run_ddd_regression()` | Checkpoint 7: triple-interaction + second-stage mechanism DDD (**secondary**, extensive margin), now precision-weighted with a dropped-occupation diagnostic | `fixest` |
| `hours_ddd_regression.R` | `run_hours_ddd_regression()` | Checkpoint 11: **primary** triple-interaction DDD on hours, pure occupation-level exposure — see `docs/decisions/hours-ddd-pivot.md` | `fixest` |
...
| `robustness/age_balance_robustness.R` | `diagnose_gilnk_by_quartile()`, `run_ddd_age_interacted()`, `build_gilnk_rake_weights()`, `run_ddd_reweighted()` | Diagnoses and builds two comparison specs for the `GilNK` imbalance found in the balance test | `tidyverse`, `fixest` |
| `robustness/pretrend_wald_test.R` | `run_pretrend_joint_test()` | Joint Wald test on `Diagnostics.R`'s pre-2020 Mother:year coefficients | `fixest` |
| `robustness/phase2_robustness.R` | `run_ddd_twoway_cluster()`, `run_ddd_education_checks()`, `run_ddd_weights_check()` | Layered on the reweighted spec above; **not wired into `main.R`** — `run_ddd_weights_check()` applies `MishkalSofi` as a `feols()` weight, which needs explicit sign-off per CLAUDE.md before use (see `docs/decisions/age-balance-robustness-chain.md`) | `tidyverse`, `fixest` |
...
|---|---|---|
| Survey weights (`MishkalSofi`, etc.) are not applied in any outcome regression | `README.md` Known Limitations, `CLAUDE.md` | Deliberate, not an oversight — do not add without raising it first. `build_exposure_cells()` is the one exception (weights the exposure regressor's own construction, not a survey-representativeness correction). `robustness/phase2_robustness.R`'s `run_ddd_weights_check()` applies `MishkalSofi` as a `feols()` weight and is a **second, not-yet-approved** exception — flagged, not wired into `main.R`. See `docs/decisions/age-balance-robustness-chain.md`. |
...
| Hours-DDD pivot is documented as the project's primary specification, but the code still gates it behind `RUN_HOURS_DDD_PIVOT <- FALSE` | `docs/decisions/hours-ddd-pivot.md` | Documentation/code lag, not a design gap — flipping the default and updating `main.R` §8g's comments is a separate, not-yet-scheduled follow-up code task. |
...
| `GilNK` (age-group) imbalance between Mother and non-Mother, concentrated in the lowest `WFH_Exposure` quartile | `docs/decisions/age-balance-robustness-chain.md` | Confirmed against real data (2026-09-09): the gap is largest in Q1 (~0.8 age-group units, t≈-81) and shrinks/reverses by Q4. Two comparison specs exist (age-interacted; `GilNK`-reweighted) but neither has replaced `main.R`'s secondary (employment) DDD spec — that's a separate, still-open decision. Whether this age-balance logic also needs to be ported to the primary (hours) DDD is a further open question (see `docs/decisions/hours-ddd-pivot.md`). |
