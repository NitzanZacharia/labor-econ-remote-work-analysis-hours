...
| Shared Statistical Helpers | `clustered_se.R`, `imbens_manski_ci.R`, `ddd_mde_diagnostics.R`, `wfh_first_stage_check.R`, `ddd_collinearity_diagnostics.R` | `clustered_se()` — cluster-robust SEs for the descriptive/figure layer (adopting it moved intervals only, never point estimates). `imbens_manski_ci()` — shared partial-identification CI solver for both Lee-bounds files. `compute_ddd_mde()` — closed-form minimum detectable effect, so a null can be read against what the design could detect. `check_wfh_first_stage_relevance()` — tests that `WFH_Exposure` actually predicts realized WFH, the shift-share design's relevance assumption. `check_spec1_collinearity()`/`check_for_dropped_coefficients()` — runtime collinearity guards. |
...
| `build_ddd_event_study_plot.R` | `build_ddd_event_study_plot()` | ggplot event-study plot (point estimates + 95% CI, zero line, treatment-boundary rule) for the above; returns a ggplot, so both exporters handle it like every other figure rather than needing `main.R` to manage a device as §7's two `iplot()` event studies do | `tidyverse` |
