...
| `scripts/tex_coef_cell.R`, `scripts/format_tex_table_body.R`, `scripts/build_paper_tables.R`, `scripts/export_paper_tables.R` | `tex_coef_cell()`, `format_tex_table_body()`, `build_paper_tables()`, `export_paper_tables()` | The generated-table layer: every paper table is assembled from the fitted models (coefficients keyed by name, fixest's own 4-significant-digit formatting) and written to `paper/tables/*.tex`, which `paper.tex` `\input{}`s. `auto_notes.tex` carries the generated note sentences (which coefficients, if any, a column dropped for collinearity). |
