...
| `scripts/hours_descriptive_plots.R` | `build_hours_descriptive_plots()` | The primary outcome described rather than estimated: the raw hours 2×2 (DiD **+0.266**, SE 0.098, against the estimated 0.2280) and a by-year series whose gap panel shows the pre-period is flat once the hours population is harmonized (Checkpoint 13). Accepts `run_hours_diagnostics()`'s own cell means so the figure and `outputs/hours_diagnostics_hours_by_period.csv` cannot disagree. |
