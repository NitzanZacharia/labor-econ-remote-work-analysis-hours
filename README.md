...
Dependencies: R with the `tidyverse` and `fixest` packages (no lockfile/renv — just install both), plus `fwildclusterboot` (which pulls in `dqrng`) for the wild-cluster-bootstrap p-values on the hours DDD (`robustness/hours_ddd_inference.R`, added 2026-09-22 — see `docs/decisions/grade-report-response.md`). The bootstrap is the only thing that needs it: it is loaded inside the function with `requireNamespace()`, and the test suite skips the bootstrap tests when it is absent. Do not add any other dependency without flagging it first (project convention).
