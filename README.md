...
- the same **DDD mechanism regression on employment** (`Employed ~ Mother×Post×WFH_Exposure`, cell-based exposure) as the secondary specification, cross-referenced against literature anchors (Bloom; Cohen & Manor 2024). The three occupation-level robustness variants of this secondary DDD (calibrated/external/realized exposure, `ddd_regression.R`) were deprecated and removed — see `docs/decisions/employment-ddd-robustness-removal.md` (deleted 2026-09-24, in git at `c800efe^`),
- a **gender placebo** test (fathers vs. childless men) to check the effect is motherhood-specific rather than a general parenthood/macro shift — an insignificant β₃ here supports the motherhood-specific reading. Both placebos run unconditionally in `main.R` and export their DiD and DDD tables: the hours (primary) one via `run_hours_gender_placebo()`, and the employment (secondary) one via `run_gender_placebo()`, wired in 2026-09-19 so the secondary margin has the same falsification check as the primary,
...
Dependencies: R with the `tidyverse` and `fixest` packages (no lockfile/renv — just install both), plus `fwildclusterboot` (which pulls in `dqrng`) for the wild-cluster-bootstrap p-values on the hours DDD (`robustness/hours_ddd_inference.R`, added 2026-09-22 — see `docs/decisions/grade-report-response.md`, deleted 2026-09-24, in git at `c800efe^`). The bootstrap is the only thing that needs it: it is loaded inside the function with `requireNamespace()`, and the test suite skips the bootstrap tests when it is absent. Do not add any other dependency without flagging it first (project convention).

```r
install.packages(c("tidyverse", "fixest", "fwildclusterboot"))
```
...
This runs the full default pipeline — load/validate data, comparative stats, the primary intensive-margin regression plus its Lee (2009) selection-bounds correction, the primary hours DDD (occupation-level exposure, calibrated/external/realized variants, a fully saturated fixed-effects version and a quartile-binned version) plus its Lee bounds, the outcome-coding and child-age heterogeneity checks, the occupation-level first stage, the wild-cluster bootstrap and the permutation test (`N_PERMUTATIONS = 999` refits, several minutes; set it to 99 for a dev run), the three secondary employment regressions (pooled, Jewish, Arab), child-age descriptives, diagnostics, and the full WFH-exposure/secondary-DDD analysis (four exposure measures, an ISCO-masking sensitivity check, the secondary cell-based DDD with a runtime collinearity diagnostic) — then writes every result to `outputs/` and every paper table to `paper/tables/` (see below). Two pieces of the empirical strategy are deliberately **not** wired into this default run and must be invoked manually:

```r
# Robustness check: full sample vs. Muasak-observed-only
source("main.R"); basic_reg_comp(cleaned_df)

# Gender placebo, employment outcome (loads and validates a separate male subsample).
# The hours placebo is NOT here: main.R already runs run_hours_gender_placebo() unconditionally
# and exports hours_gender_placebo_{did,ddd}_table.csv. Only this employment one is manual.
source(file.path("scripts", "gender_placebo.R")); run_gender_placebo(folder_path)
```

A third, fully separate script, `run_mismatch.R` (`Rscript run_mismatch.R`), runs a descriptive-only mismatch exhibit against the `cleaned_df.rds` cache that `main.R` writes beside the raw CSVs — it requires `data/israeli_cbs_wfh_2digit.csv`, which (unlike the raw CBS CSVs) *is* included in this repo, and it writes `outputs/israeli_market_mismatch.csv`.

> **Do not cite this exhibit's numbers in the paper.** It runs on `cleaned_df` (women 25–59 only), whereas the pipeline's calibrated exposure is built on the broader `exposure_population_df` (women *and* men, so an occupation's score isn't constructed from exactly the people in the regression). The two therefore differ: teaching's realized WFH share is 0.063 here and 0.069 in `outputs/wfh_exposure_calibrated.csv`. The paper uses the latter. The archived draft's "0.063" came from this exhibit and was one of the figures corrected during the pivot.
...
Runs the `testthat` suite in `tests/testthat/` (data processing, validation, schema-drift, every regression function, export, and a full-pipeline smoke test) against fixture CSVs checked into the repo, exiting non-zero on any failure. Any change touching a function used elsewhere (`data_processing.R`, the controls list) should be green on this suite before being considered done. One test is skipped by default: the block of `test-wfh_crosswalk_integrity.R` that rebuilds `data/israeli_cbs_wfh_2digit.csv` from its two public source files. To run it, set `WFH_DN_OCCUPATIONS_CSV` and `WFH_BLS_ISCO_SOC_XLS` to local copies of those files (it checks their SHA-256 first, and needs `readxl`, which is used nowhere else).

## Project structure

`main.R`, `run_tests.R`, and `run_mismatch.R` are top-level entry-point scripts and stay at the repo root; every single-function file lives under `scripts/`. `robustness/` holds the Phase 1b/1c robustness-chain scripts, covering both the primary (hours) and secondary (employment) DDDs (`balance_test.R`, `age_balance_robustness.R`, `pretrend_wald_test.R`, and, since 2026-09-22, `hours_ddd_inference.R` for the wild bootstrap and the permutation test) — each defines several related functions, so they don't fit `scripts/`'s one-function-per-file convention; `pretrend_wald_test.R` runs unconditionally (its F-statistics are the paper's parallel-trends evidence); `balance_test.R`/`age_balance_robustness.R` run under `RUN_AGE_BALANCE_ROBUSTNESS` and the permutation test under `RUN_PERMUTATION_TEST`, both defaulting to `TRUE` so a default run reproduces everything the paper cites. A further robustness script, `phase2_robustness.R`, was deprecated and removed — see `docs/decisions/employment-ddd-robustness-removal.md` (deleted 2026-09-24, in git at `c800efe^`). `data/` holds small, versioned external inputs the pipeline needs: the Dingel & Neiman teleworkability score at the two-digit ISCO-08 level (`israeli_cbs_wfh_2digit.csv`) and the ISCO-08 two-digit titles used by the appendix table (`isco08_2digit_labels.csv`). The score file is Dingel & Neiman's (2020) published binary indicator (`occupations_workathome.csv` in their replication repository, github.com/jdingel/DingelNeiman-workathome), mapped to ISCO-08 through the Bureau of Labor Statistics' 2012 crosswalk between ISCO-08 and the 2010 SOC (bls.gov/soc/soccrosswalks.htm; the author's copy was supplied by the Israeli CBS) and averaged without weights over the matched pairs within each two-digit group. Rebuilding it from those two public files reproduces it exactly; the derivation, input hashes and executable proof are in `docs/decisions/wfh-crosswalk-provenance.md` and `tests/testthat/test-wfh_crosswalk_integrity.R`. The two source files are not committed (D&N's repository is GPLv3; the BLS file is an `.xls`).
...
| `scripts/gender_placebo.R` | `run_gender_placebo()`, `run_gender_ddd_placebo()` | Reruns `basic_reg()` and the employment DDD on the male subsample (fathers vs. childless men), as a placebo for the motherhood-specific interpretation (secondary/employment outcome). Called by default from `main.R` since 2026-09-19; accepts a preloaded `cleaned_men` so it reuses the frame built for the exposure population rather than reloading the raw CSVs. Exports `gender_placebo_{did,ddd}_table.csv`. |
| `scripts/hours_gender_placebo.R` | `run_hours_gender_placebo()` | Same placebo idea, primary (hours) outcome: reruns `run_intensive_margin_reg()` plus an hours-outcome DDD placebo on the male subsample. **Called by default** from `main.R`; exports `hours_gender_placebo_{did,ddd}_table.csv`. |
| `scripts/placebo_male_frame.R` | `prepare_placebo_male_inputs()` | Loads, validates and caches the male subsample and builds the exposure inputs both gender-placebo runners share, so neither reloads the raw CSVs itself. |
...
| `scripts/lee_trim_proportion.R`, `scripts/lee_trim_cell.R` | `lee_trim_proportion()`, `lee_trim_cell()` | The Lee-bounds arithmetic both Lee-bounds files share: the excess-selection trim proportion from the four (Mother, Post) selection rates, and the top-/bottom-trim of one cell's hours distribution for the lower/upper bound. |
...
| `scripts/hours_diagnostics.R` | `run_hours_diagnostics()` | The hours (primary) counterparts: 2×2 cell means of weekly hours and the hours event study. Returns tidy per-year coefficients (via `tidy_event_study_coefs()`) rather than drawing; `main.R` plots them with `build_event_study_plot()`. |
| `scripts/tidy_event_study_coefs.R` | `tidy_event_study_coefs()` | Turns a fitted `i(ShnatSeker, …)` event study into a tidy one-row-per-year frame (estimate, SE, t, p, 95% CI, pre/post). Shared by the DiD and DDD event studies so both use one implementation of the t-on-(G−1) arithmetic. |
| `scripts/build_event_study_plot.R` | `build_event_study_plot()` | ggplot event-study figure: point estimates with 95% CIs, zero line, treatment-boundary rule, and the omitted reference year pinned as a hollow zero. Draws both Figure 6 (DiD) and Figure 7 (DDD) of the paper. |
| `scripts/hours_ddd_event_study.R` | `run_hours_ddd_event_study()` | Year-by-year event-study version of the primary hours DDD (`ShnatSeker` x `Mother` x `WFH_Exposure`, occupation-clustered), the DDD-level parallel-trends check that the two DiD-level pretrend models cannot provide. See `docs/decisions/ddd-event-study.md`. |
| `scripts/descriptive_table.R` | `build_descriptive_table()` | Table 1 of the paper: sample composition, employment rate, weekly hours and the full categorical composition of the controls, split by mother status. Unweighted. Level labels for the coded controls come from the CBS codebook. |
| `scripts/wfh_exposure_cells.R` | `build_exposure_isco2()`, `calibrate_isco_exposure()`, `build_exposure_cells()` | Builds the exposure measures: the Dingel & Neiman crosswalk, the Israeli calibration (which occupations get swapped to realized values, and why), and the pre-period shift-share index by demographic cell used by the secondary employment DDD. One of the few multi-function files in `scripts/`. |
| `scripts/hours_subgroup_comparison.R` | `build_hours_subgroup_comparison()` | Assembles the subgroup forest plots and their backing tables (all women, Jewish, Arab, men placebo) for both the hours DiD and DDD. |
| `scripts/ddd_mde_diagnostics.R` | `compute_ddd_mde()` | Closed-form minimum detectable effect for a triple interaction. Its return value is a list of scalars, which `export_all_results()` skips; since 2026-09-19 `main.R` writes the hours MDEs (with the regressor SD/IQR scalings and, since Checkpoint 15, the t(39) multiplier) to `outputs/mde_hours.csv`. |
...
| `scripts/hours_descriptive_plots.R` | `build_hours_descriptive_plots()` | The primary outcome described rather than estimated: a raw hours 2×2 and a by-year series whose gap panel shows the pre-period is flat once the hours population is harmonized (Checkpoint 13). Accepts `run_hours_diagnostics()`'s own cell means so the figure and `outputs/hours_diagnostics_hours_by_period.csv` cannot disagree. **Only the by-year plot is a paper figure** — the 2×2 was cut (see `docs/decisions/paper-figure-layer.md`); it still reaches `outputs/` as a PNG. |
...
| `scripts/wfh_share_by_year.R` | `build_wfh_share_by_year()` | Realized WFH share among employed women by survey year (`WFH` and `WFH_RefWeek`, both NA before 2021 so the result is post-period only), IDPUF-clustered SEs. The Israeli counterpart of the US workday shares the Introduction cites. Added by the 2026-09-22 editorial audit. |
| `scripts/absence_by_exposure_quartile.R` | `build_absence_by_exposure_quartile()` | Share of the employed who did not work in the reference week (`AvadBeshavua != 1`), by quartile of occupation-level exposure and by quartile × Mother × Post, on the same quartile edges as the dose-response figure. Evidence for the Limitations paragraph on the narrowed hours estimand. Added by the 2026-09-22 editorial audit. |
...
| `scripts/occupation_exposure_breaks.R` | `compute_occupation_exposure_breaks()` | Pre-period quartile edges of the occupation-level exposure score, the one rule `hours_dose_response.R`, `absence_by_exposure_quartile.R` and `hours_ddd_binned.R` all share (2026-09-22). |
| `scripts/assign_wfh_quartile.R` | `assign_wfh_quartile()` | The one `cut()` every quartile consumer (dose-response figure, binned DDD, Lee bounds, balance and sorting checks, age-balance chain) applies to a set of exposure breaks, so the bins cannot drift apart. |
| `scripts/hours_ddd_saturated.R` | `run_hours_ddd_saturated()` | The hours DDD with occupation × year, occupation × mother and mother × year fixed effects, so only the triple interaction carries a functional form. Table 2 column (3). |
| `scripts/hours_ddd_binned.R` | `run_hours_ddd_binned()` | The hours DDD with the exposure score replaced by its quartile (Figure 2's bins): Q2/Q3/Q4-vs-Q1 triple terms, plus occupations per bin. Table 2 column (4). |
| `scripts/hours_ddd_by_child_age.R` | `run_hours_ddd_by_child_age()` | Hours DiD/DDD by age of the youngest child (`GilYeledTzairMBNK` bins), each bin's mothers against all childless women — the mechanism's most direct test. Results §5.6. |
| `scripts/wfh_occupation_first_stage.R` | `build_wfh_occupation_first_stage()` | Occupation-level first stage (calibrated score vs realized 2021–23 reference-week WFH, men and women pooled) and the 40-row appendix table of external/realized/calibrated scores with the swap flag. |
| `robustness/hours_ddd_inference.R` | `run_hours_ddd_wild_bootstrap()`, `run_hours_ddd_permutation_test()` | Wild-cluster-bootstrap p-values (`fwildclusterboot`) for the headline and the DDD pre-trend coefficients, and the permutation test that reassigns the forty exposure scores across occupations (the paper's falsification test). |
| `scripts/build_permutation_plot.R` | `build_permutation_plot()` | Histogram of the permutation draws with the observed t marked. |
| `scripts/tex_coef_cell.R`, `scripts/format_tex_table_body.R`, `scripts/build_paper_tables.R`, `scripts/export_paper_tables.R` | `tex_coef_cell()`, `format_tex_table_body()`, `build_paper_tables()`, `export_paper_tables()` | The generated-table layer: every paper table is assembled from the fitted models (coefficients keyed by name, a fixed number of decimals per table: three for the hours tables, four for the employment table) and written to `paper/tables/*.tex`, which `paper.tex` `\input{}`s. `auto_notes.tex` carries the generated note sentences (which coefficients, if any, a column dropped for collinearity). |
| `scripts/hours_ddd_swap_control.R` | `run_hours_ddd_swap_control()` | Fixed-sample test of the exposure calibration: the external index as regressor on all forty occupations, with the ten swapped occupations given their own `Mother * Post` terms. Table 4, *Exposure measure*. (2026-09-23 grade-report-2 response.) |
| `scripts/exposure_sorting_check.R` | `run_exposure_sorting_check()` | Occupational-sorting diagnostic: a DiD with the occupation-level exposure score (and a top-quartile indicator) as the *outcome*, plus its event-study form, on the hours DDD's sample. Table 4, *Occupational sorting*; Results §5.3. |
| `scripts/hours_ddd_cell_exposure.R` | `run_hours_ddd_cell_exposure()` | The hours DDD on the pre-period demographic-cell exposure, which a woman's post-2021 occupation cannot move; clustered on the demographic cell, reported per SD. The sorting bound. |
| `scripts/hours_ddd_leave_one_out.R`, `scripts/build_leave_one_out_plot.R` | `run_hours_ddd_leave_one_out()`, `build_leave_one_out_plot()` | Forty refits of the primary DDD with one occupation removed each time, the range and the influence summary; and the influence plot (Appendix Figure A1). |
| `scripts/build_balance_by_exposure_quartile.R` | `build_balance_by_exposure_quartile()` | Pre-period mother-minus-childless differences in every control by quartile of the occupation-level score, IDPUF-clustered SEs (Appendix Table A2). |
...
Because `paper.tex` and `paper/appendix.tex` reference those figures with `../outputs/figures/`-relative paths and no `\graphicspath`, **the figures must exist before the paper compiles** — run `Rscript main.R` before editing `paper.tex`, and the paper only compiles from inside `paper/`.

The paper's tables are generated the same way. `main.R` ends by calling `build_paper_tables()` on the fitted models and `export_paper_tables()`, which writes one `tabular` block per table to `paper/tables/<name>.tex` plus `paper/tables/auto_notes.tex`; `paper.tex` keeps the `table` floats, captions, labels and notes by hand and `\input{}`s only the blocks. **Never edit `paper/tables/*.tex` by hand** — change `scripts/build_paper_tables.R` and re-run. The files are tracked in git so the paper compiles from a fresh clone. Two of the generated blocks, `tab_lee_bounds.tex` and `tab_lee_selection.tex`, are not `input{}` anywhere: `paper/appendix.tex` quotes their numbers in prose, and the files are kept as the provenance for those quotes.

> **The paper and the results digest are in sync with `outputs/`.** Headline figures: pre-trend *F* = 0.949 (*p* = 0.387), hours DiD 0.2280 (SE 0.1809, a null), headline DDD 3.224 (SE 1.022) with Imbens–Manski $[1.021, 5.324]$. `paper/notes/results_digest.md` is the authoritative record of every reported number and of what is and isn't claimable — including which quantities remain console-only.

## Known limitations

- **Survey weights are intentionally not applied in any regression.** CBS weight columns (`MishkalSofi`, `MishkalShnati`, etc.) exist in the raw data and are deliberately excluded from every regression in this repo (`basic_reg()`, the intensive-margin/DDD models, the pretrend model, etc.) — this is a scope decision, not an oversight, and should not be "fixed" without a separate discussion. Reported rates and regression coefficients are unweighted estimates on the analysis sample, not population-representative statistics. (Four uses of a `weights =` argument do exist, and none of them is a survey weight. `build_exposure_cells()` in `wfh_exposure_cells.R` weights by `MishkalSofi` when aggregating occupation exposure up to demographic cells — internal to building the exposure regressor, not a representativeness correction. `run_ddd_reweighted()` and `run_hours_ddd_reweighted()` in `robustness/age_balance_robustness.R` pass `weights = ~rake_weight` to `feols()`, which is the entire point of those two age-rebalancing specs; the reweighted row of the paper's robustness table comes from them. `run_hours_ddd_regression()` uses inverse-variance weights in the second-stage `lm()` over per-occupation estimates. And `build_wfh_occupation_first_stage()` weights its forty-occupation first-stage slope by occupation cell size. The claim above is specifically about CBS *survey* weights in an outcome regression, and that still holds everywhere.)
...
Before implementing anything, read (in this order): [`docs/ROADMAP.md`](docs/ROADMAP.md) (the checkpoint in question), [`docs/LLD.md`](docs/LLD.md) (schema/contracts), [`docs/HLD.md`](docs/HLD.md) (why the gap exists). For how to test something, `tests/testthat/` (run via `Rscript run_tests.R`) is the source of truth. Parts 1–2 of the original research plan (`docs/motherhood_penalty_wfh_research.md`: research question, literature review, initial empirical design) are folded into the "Background" and "Research design" sections above; Parts 3–4 (advisor feedback and the literal specification) stay in that file because the decision memos cite them by section. `docs/LLD.md`/`docs/HLD.md` reconcile the plan against the real codebase and are the authoritative reference for any gap between plan and implementation.
